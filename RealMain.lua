...
do
	...
	local textlabel = Instance.new("TextLabel")
	textlabel.Size = UDim2.new(1, 0, 0, 36)
	textlabel.Text = "nice one bro :troll:"
	textlabel.BackgroundTransparency = 1
	textlabel.ZIndex = 10
	textlabel.TextStrokeTransparency = 0
			...
			if callback then 
				table.insert(AutoToxic.Connections, vapeEvents.BedwarsBedBreak.Event:Connect(function(bedTable)
					if AutoToxicBedDestroyed.Enabled and bedTable.brokenBedTeam.id == lplr:GetAttribute("Team") then
						local custommsg = #AutoToxicPhrases6.ObjectList > 0 and AutoToxicPhrases6.ObjectList[math.random(1, #AutoToxicPhrases6.ObjectList)] or "How dare you break my bed >:( <name> | Project VOID on top"
						if custommsg then
					...
					elseif AutoToxicBedBreak.Enabled and bedTable.player.UserId == lplr.UserId then
						local custommsg = #AutoToxicPhrases7.ObjectList > 0 and AutoToxicPhrases7.ObjectList[math.random(1, #AutoToxicPhrases7.ObjectList)] or "Your bed gone lol <teamname> | Project VOID on top"
						if custommsg then
					...
					if deathTable.finalKill then
						...
						if killed == lplr then 
							if (not leavesaid) and killer ~= lplr and AutoToxicDeath.Enabled then
								leavesaid = true
								local custommsg = #AutoToxicPhrases3.ObjectList > 0 and AutoToxicPhrases3.ObjectList[math.random(1, #AutoToxicPhrases3.ObjectList)] or "*cough* i can't win with hacking but you can <name> your best | Project VOID on top"
								if custommsg then
							...
							if killer == lplr and AutoToxicFinalKill.Enabled then 
								local custommsg = #AutoToxicPhrases2.ObjectList > 0 and AutoToxicPhrases2.ObjectList[math.random(1, #AutoToxicPhrases2.ObjectList)] or "L Bad <name> | Project VOID on top"
								if custommsg == lastsaid then
									custommsg = #AutoToxicPhrases2.ObjectList > 0 and AutoToxicPhrases2.ObjectList[math.random(1, #AutoToxicPhrases2.ObjectList)] or "Skill Issues Lol <name> | Project VOID on top"
								else
									lastsaid = custommsg
								end
								if custommsg then
				...
				table.insert(AutoToxic.Connections, vapeEvents.MatchEndEvent.Event:Connect(function(winstuff)
					...
					if myTeam and myTeam.id == winstuff.winningTeamId or lplr.Neutral then
						...
						if AutoToxicWin.Enabled then
							textChatService.ChatInputBarConfiguration.TargetTextChannel:SendAsync(#AutoToxicPhrases.ObjectList > 0 and AutoToxicPhrases.ObjectList[math.random(1, #AutoToxicPhrases.ObjectList)] or "Report Me Lol | Project VOID on top")
						end
					end
				end))
				table.insert(AutoToxic.Connections, vapeEvents.LagbackEvent.Event:Connect(function(plr)
					if AutoToxicLagback.Enabled then
						...
						local msg = custommsg or "Imagine GOT LAGBACKING L "..(plr.DisplayName or plr.Name).." | Project VOID on top"
					...
					if AutoToxicRespond.Enabled then
						...
						if plr and plr ~= lplr and not alreadyreported[plr] then
							...
							if reportreason then 
								...
								local msg = custommsg or "Bozo..... I don't care about the fact that I'm hacking, I care about YOU STILL BAD AT ME. L "..(plr.DisplayName or plr.Name).." | Project VOID on top"
								textChatService.ChatInputBarConfiguration.TargetTextChannel:SendAsync(msg)
							end
						end
					end
...
runFunction(function()
    ...
    function tweenToNearestBed()
        ...
        if nearestBed and not hasTeleported then
            ...
            local tween = TweenService:Create(lplr.Character.HumanoidRootPart, TweenInfo.new(0.73), {CFrame = nearestBed.CFrame + Vector3.new(0, 2, 0)})
            tween:Play()
        end
    end
...
runFunction(function()
    local UI = {Enabled = false}
    UI = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
        Name = "TextureUI",
        Function = function(callback)
            if callback then
...
local optionsButton = GuiLibrary.ObjectsThatCanBeSaved.UtilityWindow.Api.CreateOptionsButton({
    Name = "Sing You've Been Trolled",
    HoverText = "",
...
function notify(text)
    local frame = GuiLibrary["CreateNotification"]("ProjectVOID", text, 5, "assets/InfoNotification.png")
...
notify("Mobile Button Maybe Didn't Load I Appreciate For That")
notify("Have Fun...")
notify("Project VOID Loading..")
